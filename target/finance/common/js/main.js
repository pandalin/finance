...
$(document).ready(function() { 	
	...
	if ($.cookie(COOKIE_NAME)) {
		mainNavLinkAll.each(function(index,item) {
			var $this = $(item);
			if($this.attr('href') == $.cookie(COOKIE_NAME)) {
				$this.addClass('current');
				var ulElem = $this.closest('ul');
				if(ulElem.hasClass('sub')) {
					ulElem.prev('a.hasUl').addClass('drop');
	...
	mainNavLink.click(function(event) {
		...
		if($this.hasClass('hasUl')) {
			...
			//必须加上path
			$.cookie(COOKIE_NAME,$this.attr('href') ,{path:'/',expires: 1});
		}
	});
 ...
 * successfn 成功回调函数
 */
jQuery.ajaxData=function(options,successfn) {
	if(options.url) {
		...
		$.ajax({
			...
			error:function(XMLHttpRequest, textStatus, errorThrown){
				if (textStatus === "error") {
					art.dialog.tips("%>_<%您要访问的界面出错了,过会再来吧~").time(5);
				}
			}
		});
	}
...
/**
 * ajax封装--返回{success:xxx,message:xxx,data:xxx}
 * 				success:是否成功,message:错误信息,data:数据
 * options 参数
 * successfn 成功回调函数
 */
jQuery.ajaxAction=function(options,successfn) {
	if(options.url) {
		var data = options.data || {"date": new Date().getTime()};
		$.ajax({
	    	async:options.async || false,
	        ...
	        success: function(responseData){
	        	if (!responseData.success) {
	        		art.dialog.tips(responseData.message);
	        	} else {
	        		if($.isFunction(successfn)) {
	        			successfn(responseData);
	        		}
	        ...
	        error:function(XMLHttpRequest, textStatus, errorThrown){
	        	if (textStatus === "error") {
	        		art.dialog.tips("%>_<%您要访问的界面出错了,过会再来吧");
	        	}
	        }
	    });
	}
